...
public class MidiProcessor {
    ...
    public static ArrayList<Note> toLyreNoteList(ArrayList<Note> noteList, int transposition, int[] groupMapping, InvalidKeySetting blackKeySetting,boolean reserveNoteOff) {
        ArrayList<Note> lyreNotes = new ArrayList<>();
        for (Note note : noteList) {
            if (reserveNoteOff || note.isNoteOn()) {
                //88键 ======> 36键 ========> 21键
                int note_36 = to36Key(note.getValue() + transposition, groupMapping);
                if (note_36 != -1) {
    ...
    public static ArrayList<Note> toOldLyreNoteList(ArrayList<Note> noteList, int transposition, int[] sp, InvalidKeySetting invalidKeySetting,boolean reserveNoteOff) {
        ArrayList<Note> lyreNotes = new ArrayList<>();
        for (Note note : noteList) {
            if (reserveNoteOff || note.isNoteOn()) {
                //88键 ======> 36键 ========> 21键
                int note_36 = to36Key(note.getValue() + transposition, sp);
                if (note_36 != -1) {
